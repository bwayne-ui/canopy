...
async function main() {
  ...
  const reportSeed = [
    ...
    { code: 'RPT-011', name: 'Cash Flow Forecast', category: 'Treasury', frequency: 'Weekly', format: 'Excel', recipients: 'Treasury, CFO', querySource: 'prisma', queryLogic: JSON.stringify({ model: 'cashFlow', orderBy: { transactionDate: 'desc' } }), parametersSchema: JSON.stringify([{ name: 'horizon_days', type: 'number', default: 30 }]), visibility: 'Team', minGrade: 'P3' },
    { code: 'RPT-012', name: 'Investor Activity Report', category: 'Investor Services', frequency: 'Monthly', format: 'PDF', recipients: 'IR Team', querySource: 'prisma', queryLogic: JSON.stringify({ model: 'investor' }), parametersSchema: JSON.stringify([{ name: 'entityName', type: 'string', required: false }]), visibility: 'Team', minGrade: 'P1' },
    { code: 'RPT-013', name: 'AML/KYC Status Report', category: 'Compliance', frequency: 'Quarterly', format: 'Dashboard', recipients: 'Compliance', querySource: 'sql', queryLogic: 'SELECT * FROM Investor WHERE status = "Active"', parametersSchema: JSON.stringify([]), visibility: 'Team', minGrade: 'P4' },
    { code: 'RPT-014', name: 'Trial Balance', category: 'Accounting', frequency: 'Monthly', format: 'Excel', recipients: 'Fund Accounting', querySource: 'sql', queryLogic: 'SELECT * FROM CashFlow ORDER BY transactionDate DESC', parametersSchema: JSON.stringify([{ name: 'period', type: 'month', required: true }]), visibility: 'Team', minGrade: 'P2' },
    { code: 'RPT-015', name: 'Side Letter Compliance Tracker', category: 'Legal', frequency: 'On-Demand', format: 'Excel', recipients: 'Legal, IR', querySource: 'skill', queryLogic: JSON.stringify({ skill: 'side-letter-mfn-scan' }), parametersSchema: JSON.stringify([{ name: 'entityId', type: 'string', required: true }]), visibility: 'Team', minGrade: 'P3' },

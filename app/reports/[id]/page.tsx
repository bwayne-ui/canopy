...
import { Play, Save, ArrowLeft, Lock, Users, Building, Globe2 } from 'lucide-react';
...
export default function ReportDetail() {
  ...
  const [user, setUser] = useState<{ name: string; role: string }>({ name: 'billywayne', role: 'Controller' });
  ...
  const load = () => fetch(`/api/reports/${id}`).then((r) => r.json()).then((d) => {
    ...
    setEditRequiredRole(d.requiredRole ?? '');
  ...
  const save = async () => {
    ...
    const r = await fetch(`/api/reports/${id}`, {
      ...
      body: JSON.stringify({ name: editName, description: editDesc, visibility: editVisibility, requiredRole: editRequiredRole || null, queryLogic: editQueryLogic, parametersSchema: parsedParams }),
    });
  ...
  const run = async () => {
    ...
    const r = await fetch(`/api/reports/${id}/run`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ user: { id: 'demo-user', name: user.name, role: user.role }, parameters: paramValues }),
    });
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-4 space-y-3">
          ...
          {!editing ? (
            <dl className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs">
              ...
              <div><dt className="text-gray-400 text-[10px] uppercase">Required Role</dt><dd>{report.requiredRole ?? '—'}</dd></div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <select value={editRequiredRole} onChange={(e) => setEditRequiredRole(e.target.value)} className="px-2 py-1 border rounded">
                  <option value="">— No role required —</option>
                  {['Analyst', 'IR', 'Tax', 'Legal', 'Compliance', 'Treasury', 'Fund Accountant', 'Manager', 'Controller', 'CFO', 'Admin'].map((v) => <option key={v}>{v}</option>)}
                </select>
              </div>
            </div>
          )}
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 space-y-3">
          <h3 className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Run Report</h3>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-1">
              ...
              <select value={user.role} onChange={(e) => setUser({ ...user, role: e.target.value })} className="px-2 py-1 text-xs border rounded">
                {['Guest', 'Analyst', 'IR', 'Tax', 'Legal', 'Compliance', 'Treasury', 'Fund Accountant', 'Manager', 'Controller', 'CFO', 'Admin'].map((r) => <option key={r}>{r}</option>)}
              </select>
            </div>
          </div>
